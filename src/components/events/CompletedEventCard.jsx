...
const CompletedEventCard = ({ completedEvent }) => {
  const [isCompletedEventHovered, setIsCompletedEventHovered] = useState(false);
  const [isCompletedEventModalOpen, setIsCompletedEventModalOpen] = useState(false);
  const [completedEventActiveTab, setCompletedEventActiveTab] = useState('description');
  ...
  return (
    <>
      ...
      <div className="completed-event-wrapper">
        ...
        <motion.div 
          className="completed-event-interactive-container"
        >
          ...
          <motion.div
            ...
            transition={{ 
              ...
              y: { duration: 0.4, ease: [0.175, 0.885, 0.32, 1.275] }
            }}
          >
            ...
            {isCompletedEventHovered && (
              <motion.div
                ...
                transition={{
                  ...
                  scaleY: { duration: 0.6, ease: [0.25, 0.46, 0.45, 0.94] }
                }}
                  ...
                  >
                    ...
                    <div className="completed-event-detail-item">
                      <div className="completed-event-icon">
                        <svg className="completed-event-svg completed-event-date-icon" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd" />
                        </svg>
                      </div>
                      <span className="completed-event-detail-text">{eventData.date}</span>
                    </div>

                    {/* Time */}
                    <div className="completed-event-detail-item">
                      <div className="completed-event-icon">
                        <svg className="completed-event-svg completed-event-time-icon" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" />
                        </svg>
                      </div>
                      <span className="completed-event-detail-text">{eventData.time}</span>
                    </div>

                    {/* Location */}
                    <div className="completed-event-detail-item">
                      <div className="completed-event-icon">
                        <svg className="completed-event-svg completed-event-location-icon" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
                        </svg>
                      </div>
                      <span className="completed-event-detail-text">{eventData.location}</span>
                    </div>
                  </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="completed-event-modal-header">
                ...
                >
                  <svg className="completed-event-close-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="completed-event-tabs">
                {['description', 'speakers', 'photos', 'sponsors'].map((tab) => (
                  <button
                    key={tab}
                    onClick={() => handleCompletedEventTabChange(tab)}
                    className={`completed-event-tab ${
                      completedEventActiveTab === tab ? 'completed-event-tab-active' : ''
                    }`}
                  >
              ...
              <div className="completed-event-tab-content">
                <AnimatePresence mode="wait">
                  {/* Description Tab */}
                  {completedEventActiveTab === 'description' && (
                    <motion.div
                      key="description"
                      ...
                      {eventData.perks && eventData.perks.length > 0 && (
                        <div>
                          <h3 className="completed-event-perks-title">
                            <svg className="completed-event-star-icon" fill="currentColor" viewBox="0 0 20 20">
                          ...
                          <ul className="completed-event-perks-list">
                            {eventData.perks.map((perk, index) => (
                              <li key={index} className="completed-event-perk-item">
                                <svg className="completed-event-check-icon" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    >
                      {eventData.speakers && eventData.speakers.length > 0 ? (
                        eventData.speakers.map((speaker) => (
                          <div key={speaker.id} className="completed-event-speaker-card">
                            <div className="completed-event-speaker-header">
                              ...
                              <div className="completed-event-speaker-info">
                                <h4 className="completed-event-speaker-name">{speaker.name}</h4>
                                <p className="completed-event-speaker-title">{speaker.title}</p>
                                <p className="completed-event-speaker-company">{speaker.company}</p>
                              </div>
                            </div>
                            <p className="completed-event-speaker-bio">{speaker.bio}</p>
                            <div className="completed-event-speaker-social">
                              ...
                              >
                                <svg className="completed-event-social-icon" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <p className="completed-event-no-data">No speakers information available.</p>
                      )}
                    </motion.div>
                  )}
                  ...
                  {completedEventActiveTab === 'photos' && (
                    ...
                    >
                      {eventData.photos && eventData.photos.length > 0 ? (
                        ...
                        <p className="completed-event-no-data">No photos available.</p>
                      )}
                    </motion.div>
                  )}
                  ...
                  {completedEventActiveTab === 'sponsors' && (
                    ...
                    >
                      {eventData.sponsors && Object.keys(eventData.sponsors).length > 0 ? (
                        Object.entries(eventData.sponsors).map(([tier, sponsors]) => (
                          <div key={tier} className="completed-event-sponsor-tier">
                        ...
                        <p className="completed-event-no-data">No sponsor information available.</p>
                      )}
                    </motion.div>
                  )}
                </AnimatePresence>
